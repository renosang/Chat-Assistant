if (window.GEMINI_CONTENT_SCRIPT_LOADED) {
  ...
  chrome.storage.sync.get(['themeMode', 'dimmerValue', 'darkModePreset', 'themeAccent', 'chatFontSize', 'allowCopy', 'autoCapitalize', 'cleanPaste'], (data) => {
  ...
  function checkMacroConnectionStatusLabel() {
    ...
    chrome.storage.sync.get(['macroAuthToken'], (data) => {
      const isLoggedOut = !data.macroAuthToken;
      let warningLabel = targetDiv.querySelector(".gemini-macro-warning-label");

      if (isLoggedOut) {
        if (!warningLabel) {
          warningLabel = document.createElement("span");
          warningLabel.className = "gemini-macro-warning-label";
          warningLabel.style.cssText = "color: #ff3b30; font-size: 12px; font-weight: 700; margin-right: 12px; display: inline-flex; align-items: center; gap: 4px; background: rgba(255, 59, 48, 0.08); padding: 4px 10px; border-radius: 6px; border: 1px solid rgba(255, 59, 48, 0.15); animation: gemini-pulse-opacity 1.5s infinite ease-in-out; align-self: center;";
          warningLabel.innerHTML = "⚠️ Cần đăng nhập để kết nối macro";

          if (!document.getElementById("gemini-pulse-opacity-style")) {
            const style = document.createElement("style");
            style.id = "gemini-pulse-opacity-style";
            style.textContent = "@keyframes gemini-pulse-opacity { 0%, 100% { opacity: 0.7; } 50% { opacity: 1; } }";
            document.head.appendChild(style);
          }

          targetDiv.insertBefore(warningLabel, targetDiv.firstChild);
        }
      } else {
        if (warningLabel) {
          warningLabel.remove();
  ...
  function openMacroSearchOverlay(triggerEl) {
    cachedMacrosList = null; // Force fresh fetch on open to reflect new channel mappings instantly
    cachedBrandContext = null;
